...
- `users/{uid}` — `email, displayName, theme ('dark'|'light'), createdAt` + preferencias: `exportPrompt, hiddenColumns, hiddenFilters, columnOrder, tagColors ({ etiqueta: índice de TAG_PALETTE }), cardMode ('simple'|'project')`
...
- **Acciones rápidas en la tarjeta del tablero** (`TaskCard.jsx`): ◀ ▶ mueven la tarea a la columna visible anterior/siguiente (`sendTaskToColumn`, al final de la columna destino), ✓ la marca como hecha (`markTaskDone` + confeti; sirve con la columna "Hecho" oculta) y copiar. El modal de tarea también tiene un botón "Hecho" que guarda y completa. Doble clic en una etiqueta abre `TagQuickEdit` (renombrar solo en esa tarjeta: nombre existente → adopta su color; nombre nuevo → pregunta el color y lo guarda en `prefs.tagColors`; vacío → la quita) y en la prioridad abre `PriorityQuickMenu`. Como el clic simple abre la tarea, en las etiquetas se retrasa 250 ms para distinguirlo del doble clic. Los popovers van en un portal (`QuickPopover`) porque las tarjetas recortan su contenido y cortan la propagación de eventos para no abrir la tarea ni iniciar un arrastre.
- **Colores de etiqueta**: `tagColor(nombre)` usa el color elegido (`setTagColorOverrides`, alimentado desde las preferencias en `useStore`) o, si no hay, uno derivado del nombre.
- **Tipo de tarjeta** (`prefs.cardMode`, menú de ajustes junto a los filtros): "Tarjeta sencilla" abre el modal de tarea con el bloque de detalles (proyecto, columna, prioridad, fecha, etiquetas, subtareas) plegado y resumido en una línea; "Tarjeta de proyecto" lo abre desplegado. Por defecto: sencilla.
